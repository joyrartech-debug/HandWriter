...
class _CanvasScreenState extends ConsumerState<CanvasScreen>
  ...
  void _showPageManager(CanvasState canvasState) {
    showModalBottomSheet(
      ...
      builder: (ctx) => _PageManagerSheet(initialState: canvasState),
    );
  }
...
// ═══════════════════════════════════════════════════════════════
//  Page Manager Bottom Sheet — supports multi-page selection
// ═══════════════════════════════════════════════════════════════

/// Modal bottom sheet widget for page management.
///
/// Owns the multi-page selection state ([_selected]) so that the action bar
/// can appear outside the scrollable grid area while reacting to the same
/// state updates.
class _PageManagerSheet extends ConsumerStatefulWidget {
  final CanvasState initialState;
  const _PageManagerSheet({required this.initialState});

  @override
  ConsumerState<_PageManagerSheet> createState() => _PageManagerSheetState();
}

class _PageManagerSheetState extends ConsumerState<_PageManagerSheet> {
  ...
  void _cutSelected(CanvasState s) {
    // Build a list of (PageData, PageEntry) for the clipboard
    ...
    // Remove the pages from the current notebook
    ref.read(canvasProvider.notifier).deletePages(_selected.toList());
    _clearSelection();
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
                      ...
                      if (_isSelecting) ...[
                        ...
                        IconButton(
                          ...
                          onPressed: () {
                            ...
                            // (user taps pages to select them)
                            // We show a hint — actual selection happens on tile long-press
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
  ...
  // ── Helpers duplicated from _CanvasScreenState for use inside the sheet ──
...
/// Bottom action bar shown when one or more pages are selected in the page manager.
...
class _PageGridReorderable extends StatefulWidget {
  ...
  /// Contains the set of currently selected document indices.
...
class _PageGridReorderableState extends State<_PageGridReorderable> {
  ...
  Widget build(BuildContext context) {
    return GridView.builder(
      ...
      itemBuilder: (ctx, visIdx) {
        ...
        // Normal mode: tap = navigate, long-press = drag OR enter select mode
        return DragTarget<int>(
          ...
          builder: (ctx2, accepted, rejected) {
            return LongPressDraggable<int>(
              ...
              onDraggableCanceled: (_, __) {
                // Long-press that didn't become a drag → enter select mode
                setState(() { _dragFromVisIdx = null; _dragOverVisIdx = null; });
              },
              feedback: Material(
                elevation: 8,
  ...
  Widget _buildTile(
    ...
    // Border colour: selected > current page > drag-over > default
